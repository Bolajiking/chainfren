...
export default function OwnershipMapDesktop({ claims, edges, layout }) {
  ...
  const [selected, setSelected] = useState(validIds.has(start) ? start : 'attention-to-ownership')
  ...
  const select = (id) => setSelected(validIds.has(id) ? id : claims[0].id)
  ...
  return (
    <section className={styles.ownershipMap} aria-labelledby="ownership-map-title">
      ...
      <div className={styles.mapViewport} onPointerDown={(event) => { drag.current = point(event); event.currentTarget.setPointerCapture(event.pointerId) }} onPointerMove={(event) => { if (!drag.current) return; const next = point(event); setView((current) => ({ ...current, x: current.x + next.x - drag.current.x, y: current.y + next.y - drag.current.y })); drag.current = next }} onPointerUp={() => { drag.current = null }}>
        <svg viewBox={`0 0 ${CANVAS.width} ${CANVAS.height}`} role="img" aria-label="Ownership claim map" aria-describedby="map-help">
