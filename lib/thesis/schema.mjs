...
export function validatePublicSystem(system, chapterSlugs = new Set()) {
  ...
  const requireSequence = (records, ids, label) => {
    ...
    records.forEach((item) => { assert(typeof item.title === 'string' && typeof item.summary === 'string', `${label} ${item.id} requires text`); validateDestination(item.href, `${label} ${item.id}`); if (item.maturity) assert(STAGES.has(item.maturity), `${label} ${item.id} has invalid maturity`) })
  }
  ...
  const privateHorizonPattern = /\b(?:\d{4}|Q[1-4]|budget|target|metric|revenue|runway|signed\s+revenue|decision-rights|control\s+matrix|risk\s+register)\b/i
...
export function validateStages(records) {
  const expected = {
    'media-launchpad': 'early-access', 'creator-growth-os': 'live-core', 'community-engine': 'live-core', 'ai-agent-studio': 'early-access',
    'creator-network': 'live', sabi: 'building', 'star-factor': 'later', indy: 'directional',
  }
  ...
  records.forEach((record) => { assert(typeof record.id === 'string' && typeof record.label === 'string' && STAGES.has(record.maturity) && isRoute(record.href), `Invalid public maturity record ${record.id}`); assert(expected[record.id] === record.maturity, `Public maturity record ${record.id} violates the required mapping`) })
}
